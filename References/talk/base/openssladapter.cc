...
OpenSSLAdapter::SSLPostConnectionCheck(SSL* ssl, const char* host) {
  ...
  // !?! (jbeda) This is a huge security hack to get us to connect to
  // buzz.troutboard.com with an @google.com JID.  The engine seems to
  // think that we should validate the cert against the JID domain.
  // However, the spec _seems_ to say that we should verify against
  // the domain name of the connecting server.  This should require
  // a reverse DNS lookup.  Also, we should use and look for the xmpp
  // altName extension in the cert and use that too to verify the cert.
